...
class inficon310C(object):
    ...
    def shutterOpen(self):
        """ Open the shutter """
        relay = 1
        open_shutter = 1 #Override relay:Turn on
        command1 = 'GE' + str(relay) + str(open_shutter)
        #Unlocks the relay back to instrument
        command2 = 'GE' + str(relay) + str(3)
        
        response1 = str(self.comm(command1))
        response2 = str(self.comm(command2))
        
        if response1 == response2 == 'A':
            return "Shutter Open"
        else:
            return "Failed"
    ...
    def shutterClose(self):
        """ Close the shutter """
        relay = 1
        close_shutter = 2 #Override relay:Turn off
        command1 = 'GE' + str(relay) + str(close_shutter)
        #Unlocks the relay back to instrument
        command2 = 'GE' + str(relay) + str(3)
